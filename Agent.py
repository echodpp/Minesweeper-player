...
class Agent(object):
    def __init__(self, env, num_of_mine):
        ...
        self.cell_to_inference = queue.Queue()
        self.cell_unresolved = queue.Queue()

    def inference(self):
        # print(self.num_mine_found, len(self.identified_hidden_cell))
                ...
                for i in range(-1, 2):
                    for j in range(-1, 2):
                        if (
                            ...
                            self.kb[x + i][y + j] = self.env.respond_query(x + i, y + j)
                            self.num_cell_left -= 1
                            self.identified_hidden_cell.append(deepcopy((x + i, y + j)))
